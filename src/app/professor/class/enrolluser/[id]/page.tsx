...
import { deleteUsersClass } from '@/services/classAdmin/deleteUsersContest';
...
export default function UserEnroll() {
  ...
  useEffect(() => {
    ...
    const newUrl = `/professor/class/enrolluser/${classId}?${query.toString()}`;
    window.history.replaceState(null, '', newUrl);
  }, [currentPage, selectedUser, classId]);
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({
        queryKey: ['classUsersListData', classId],
      });
    },
    onError: (error: any) => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (ids: number[]) => {
      const payload = { ids };
      return deleteUsersClass(classId, payload);
    },
    onSuccess: () => {
  ...
  return (
    <div className="flex flex-col min-h-screen p-8 ">
      ...
      {isStudentModalOpen && (
        <div
          className="fixed inset-0 left-0 bottom-0 right-0  z-50 flex items-center justify-center bg-black bg-opacity-50 font-semibold text-gray-700"
          onClick={closeModal}
        >
          <div
