...
import { postClassAnnouncement } from '@/services/classannouncementAdmin/postClassAnnouncement';
import { getAllClassAnnouncement } from '@/services/classannouncementAdmin/getAllClassAnnouncement';
import { deleteClassAnnouncement } from '@/services/classannouncementAdmin/deleteClassAnnouncement';

export default function EnrollClassAnnouncementPage() {
  ...
  const mutation = useMutation({
    mutationFn: (data) => postClassAnnouncement(classId, data),
    onSuccess: () => {
      message.success('공지가 성공적으로 등록되었습니다.');
      queryClient.invalidateQueries({
        queryKey: ['classAnnouncementsListData', classId],
      });
    },
    onError: (error: any) => {
  ...
  const { data: announcementsListData, isLoading: announcementsIsLoading } =
    useQuery({
      queryKey: ['classAnnouncementsListData', classId],
      queryFn: () => getAllClassAnnouncement(classId),
    });

  const announcements = announcementsListData?.data.results || [];
      ...
      <div className="w-full py-8 font-semibold bg-white shadow-lg rounded-3xl text-secondary">
        <form onSubmit={handleSubmit(onSubmit)}>
          <section className="flex items-center px-16">
            <h1 className="mb-3 text-lg md:mb-0">분반 공지 관리</h1>
          </section>
