...
export default function StudentMain() {
  ...
  const [solveGrassData, setSolveGrassData] = useState([]);
  const [solveLevelData, setSolveLevelData] = useState(null);
  const [announcementData, setAnnouncementData] = useState([]);
  const [solveTagData, setSolveTagData] = useState(null);
  const [onGoingAssignmentData, setOnGoingAssignmentData] = useState([]);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const grass = await getSolveGrass();
        setSolveGrassData(grass?.data);

        const level = await getSolveLevel();
        setSolveLevelData(level?.data);

        const announcement = await getTop3AnnouncementUser();
        setAnnouncementData(announcement?.data?.data || []);

        const tags = await getSolveTag();
        setSolveTagData(tags?.data);

        const assignments = await getOngoingAssignment();
        setOnGoingAssignmentData(assignments?.data);

        const contests = await getOngoingContest();
        setOnGoingContestData(contests?.data);
      } catch (error) {
  ...
  const solveGrass = solveGrassData || [];
  // 오늘 날짜 가져오기
  const today = new Date();
