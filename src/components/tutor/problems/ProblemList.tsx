...
export default function ProblemList() {
  ...
  useEffect(() => {
    ...
    router.push(`/tutor/problems/list?${query.toString()}`);
    refetch();
  }, [
    currentPage,
    router,
              ...
              <tbody>
                {problemList.map((item: ProblemList) => (
                  <tr
                    ...
                    key={item.id}
                    onClick={() =>
                      router.push(`/tutor/problems/list/${item.id}`)
                    }
                  >
                    ...
                    <td className="flex items-center p-4 space-x-2 text-xs sm:text-base">
                      <GoCodescan
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          router.push(
                            `/tutor/problems/submission/${item.id}?page=1`,
                          );
                        }}
                      />
                      <MdChecklist
                      ...
                      <TbEdit
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          router.push(`/tutor/problems/edit/${item.id}`);
                        }}
                      />
                      <FiTrash2
                        className="text-lg cursor-pointer lg:text-xl hover:text-gray-500"
