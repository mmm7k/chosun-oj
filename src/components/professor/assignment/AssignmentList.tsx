...
export default function AssignmentList() {
  ...
  useEffect(() => {
    router.push(`/professor/assignment/list?page=${currentPage}`);
    refetch();
  ...
  const deleteAssignmentMutation = useMutation({
    ...
    onSuccess: () => {
      message.success('과제가 성공적으로 삭제되었습니다.');
      refetch();
    },
    onError: (error: any) => {
      ...
      <div className="w-full h-full py-8 font-semibold bg-white shadow-lg rounded-3xl text-secondary">
              ...
              <tbody>
                {assignmentList.map((item: AssignmentList) => (
                  <tr
                    ...
                    key={item.id}
                    onClick={() =>
                      router.push(`/professor/assignment/list/${item.id}`)
                    }
                  >
                    ...
                    <td className="flex items-center p-4 space-x-2 ">
                      <MdOutlineLibraryAdd
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          router.push(
                            `/professor/assignment/enrollproblem/${item.id}`,
                          );
                        }}
                      />
                      ...
                      <TbEdit
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          router.push(`/professor/assignment/edit/${item.id}`);
                        }}
                      />
                      <FiTrash2
                        className="text-lg cursor-pointer lg:text-xl hover:text-gray-500"
        ...
        <section className="flex items-center w-full px-3 sm:px-16 mt-4 justify-between">
          <div className="flex space-x-3">
            <Link href="/professor/assignment/post">
              <button
            ...
            <Link href="/professor/assignment/submission">
              <button
                className="px-4 py-2 text-sm font-normal text-white bg-primary rounded-xl hover:bg-primaryButtonHover"
                type="submit"
              >
